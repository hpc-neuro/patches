...
function exp_struct = experiment(button, patch_struct)
    ...
    for i = 1:patch_struct.total/2
        if mod(i,patch_struct.trials) == 0 && i < patch_struct.total/2  % patch_struct.total
          block = block + 1;
	  % block = ceil( i / patch_struct.trials );
        	
        elseif mod(i,patch_struct.trials) == 1 && i < patch_struct.total
            if i > 1
                break_str = ...
                    sprintf('%s\n\n', ...
	...
	% left = 1   right = 0
	    ...
	    base = patch_struct.lft_ndx(i);
	else
	    base = patch_struct.rgt_ndx(i);
	end

	% high = 0    low = 1 
	exp_struct.ptch_ndx(i) = 2*(base-1)+1+exp_struct.hl_ndx(i);
	    
	ptch_name = patch_struct.ptchs(exp_struct.ptch_ndx(i)).name;
