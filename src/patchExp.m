...
function exp_struct = setExpValues(answer, tot)    
    ...
    nflips = 3;
    num_xfactors = 2;  %% (saliance, size)
    
    trial_struct = struct();
    trial_struct.target_ndx = repmat(NAN, tot, 1);  
    trial_struct.lft_ndx = repmat(NAN, tot, 1);  
    trial_struct.rgt_ndx = repmat(NAN, tot, 1);  
    trial_struct.choice = repmat(NAN, tot, 1);  
    trial_struct.confidence = repmat(NAN, tot, 1);  
    trial_struct.correct = repmat(NAN, tot, 1);  
    trial_struct.xfactors = cell(tot, num_xfactors);
